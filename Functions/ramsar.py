    ...
    try:
        ...
        if(new_links):
            for i in new_links:
                ...
                texto = soup.find('div', class_ = 'field-items').get_text()
...
def ramsar3(path):
    try:
        print('[ramsar][politicas][start]')
        titulos = []
        textos = []
        links = ['https://www.ramsar.org/about/the-convention-on-wetlands-and-its-mission', 'https://www.ramsar.org/about/the-bodies-of-the-convention', 'https://www.ramsar.org/about/transparency', 'https://www.ramsar.org/about/resource-mobilization']
        dia = datetime.today().strftime('%y%m%d')
