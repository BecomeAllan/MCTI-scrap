...
def globalwomennet1(path,keywords = 'brazil'): # A entrada é o caminho do arquivo que vai ser definido no arquivo ppfcentral.py e as palavras-chaves
  try:
    print('[globalwomennet][oportunidades][start]')
...
def globalwomennet2(path):
    try:
        print('[globalwomennet][noticias][start]')
        ...
        textos = []
        links = []
        page = Request('https://www.globalwomennet.org/news/blog/', headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.77 Safari/537.36'})
        ...
        print('[globalwomennet][noticias][end]')
